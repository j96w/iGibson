...
class Viewer:
    ...
    def update(self):
        ...
        # self.py = -3.4
        # self.pz = 1.5
        # self.view_direction = np.array([-0.3, -0.9, -0.4])

        self.px = -0.2
        self.py = -0.3
        self.pz = 1.7
        self.view_direction = np.array([0.6, 0.9, -0.7])
        ...
        # print("ext", self.ext_view_id)
        cv2.imshow("ExternalView", frame)
        cv2.imwrite('/media/jeremy/Storage/DPS_video/datageneration/ext/{0}.png'.format(self.ext_view_id), frame*255.0)
        self.ext_view_id += 1
        ...
        if self.renderer is not None:
            if self.is_robosuite:
                ...
                if len(frames) > 0:
                    frame = cv2.cvtColor(np.concatenate(frames, axis=1), cv2.COLOR_RGB2BGR)
                    # print("rob view", self.rob_view_id)
                    cv2.imshow("RobotView", frame)
                    cv2.imwrite('/media/jeremy/Storage/DPS_video/datageneration/rob/{0}.png'.format(self.rob_view_id), frame*255.0)
                    self.rob_view_id += 1
