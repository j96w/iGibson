...
class MotionPlanningWrapper(object):
    ...
    def __init__(self, env=None, base_mp_algo="birrt", arm_mp_algo="birrt", optimize_iter=0, fine_motion_plan=True, collision_obj=[]):
        """
    ...
    def setup_arm_mp(self):
        ...
        self.arm_ik_threshold = 0.1

        self.mp_obstacles = []
        self.mp_obstacles = self.collision_obj
    ...
    def dry_run_base_plan(self, path, in_hand, grasp_pose, slow=False):
        """
        ...
        if not slow:
            way_point = path
            set_base_values_with_z(self.robot_id, [way_point[0], way_point[1], way_point[2]], z=self.initial_height)
    ...
    def get_ik_parameters(self):
        ...
        if self.robot_type == "FetchGripper":
            max_limits = list(self.robot.upper_joint_limits)
            max_limits[0] = 0.0
            max_limits[1] = 0.0
            min_limits = list(self.robot.lower_joint_limits)
            min_limits[0] = 0.0
            min_limits[1] = 0.0

            rest_position = list(get_joint_positions(self.robot_id, self.robot.joint_ids))
            rest_position[0] = 0.0
            rest_position[1] = 0.0

            joint_range = list(np.array(max_limits) - np.array(min_limits))
            joint_range = [item + 1 for item in joint_range]
            joint_damping = list(self.robot.joint_damping)

        elif self.robot_type == "Fetch":
    ...
    def get_arm_joint_positions(self, arm_ik_goal, pour=False):
        """
        ...
        while n_attempt < max_attempt:
            ...
            arm_joint_positions = p.calculateInverseKinematics(
                ...
                # targetOrientation=[ -0.092296, 0.092296, 0.7010574, 0.7010574 ],
                targetOrientation=[-0.5, 0.5, 0.5, 0.5] if not pour else [0, 0.7071068, 0.7071068, 0],
            ...
            if self.robot_type == "FetchGripper":
                ...
                # arm_joint_positions = list(arm_joint_positions[5:12])
            elif self.robot_type == "Fetch":
            ...
            # print(collision_free, self.arm_joint_ids)

            # if arm_joint_positions[0] < 0.17:
            #     continue
    ...
    def dry_run_arm_plan(self, arm_path, in_hand, grasp_pose, pan_in_hand, food_on_pan, food_on_pan_grasp_pose, slow=False):
        """
        ...
        if not slow:
            ...
            if pan_in_hand and len(food_on_pan) != 0:
                for food in food_on_pan:
                    gripper_pos = p.getLinkState(self.robot_id, self.robot.end_effector_part_index())[0]
                    gripper_orn = p.getLinkState(self.robot_id, self.robot.end_effector_part_index())[1]
                    object_pose = p.multiplyTransforms(gripper_pos, gripper_orn, food_on_pan_grasp_pose[food][0], food_on_pan_grasp_pose[food][1])

                    set_pose(food, object_pose)
                    ...
                    for joint_way_point in arm_path:
                        ...
                        if pan_in_hand and len(food_on_pan) != 0:
                            for food in food_on_pan:
                                gripper_pos = p.getLinkState(self.robot_id, self.robot.end_effector_part_index())[0]
                                gripper_orn = p.getLinkState(self.robot_id, self.robot.end_effector_part_index())[1]
                                object_pose = p.multiplyTransforms(gripper_pos, gripper_orn,
                                                                   food_on_pan_grasp_pose[food][0],
                                                                   food_on_pan_grasp_pose[food][1])

                                set_pose(food, object_pose)

                        self.simulator_sync()
                        sleep(0.005)  # animation
                else:
